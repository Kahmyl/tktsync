...
docker run --rm --network host -e PGPASSWORD=tktsync "$POSTGRES_IMAGE" \
  ...
  -c "SELECT 1 / CASE WHEN EXISTS (SELECT 1 FROM pg_constraint WHERE conname='app_users_email_normalized_ck') THEN 1 ELSE 0 END AS platform_admin_email_constraint_ok"
