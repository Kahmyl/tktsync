...
echo "Running M5 Reservation lifecycle certification..."
...
echo
echo "Running M4 regression certification..."
"$ROOT/scripts/verify-m4.sh"

echo
echo "Checking M5 implementation..."
...
if find "$ROOT/backend/internal/ticketing" -type f ! -name '.gitkeep' | grep -q .; then
  echo "M5 unexpectedly implemented Ticketing-domain files."
  exit 1
fi
