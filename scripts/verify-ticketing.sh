...
echo "Running Ticketing confirmation, issuance, Ticket lifecycle, and inventory release certification..."
(
  cd "$ROOT/backend"
  go test -tags=integration ./internal/reservation ./internal/allocation ./internal/partnerapi ./internal/audit -run '^(TestTicketing|TestAudit)' -count=1
)
...
echo "Running Reservation regression certification..."
POSTGRES_PORT="$POSTGRES_PORT" DATABASE_URL="$DATABASE_URL" "$ROOT/scripts/verify-reservations.sh"

echo "Ticketing verification COMPLETE."
