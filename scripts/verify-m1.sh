...
SQL="$ROOT/tests/integration/m1_schema_invariants.sql"
...
if command -v psql >/dev/null 2>&1; then
  ...
  echo "Running M1 verification using local psql..."
...
if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1; then
  ...
  if [[ -z "$service" ]]; then
    ...
    echo "Install psql or run tests/integration/m1_schema_invariants.sql manually."
    exit 1
  fi

  echo "Running M1 verification through Docker Compose service: $service"
...
echo "  tests/integration/m1_schema_invariants.sql"
exit 1
