# TktSync Relational Data Model & Schema Specification

**Document status:** Governing Relational Data Model & Schema Specification  
...
Partner webhook signing secrets are a deliberate exception to one-way storage because TktSync must recover them to generate outbound HMAC signatures. They SHALL be stored only as authenticated ciphertext with an external encryption-key version, as defined by the Security & Authentication Specification.
