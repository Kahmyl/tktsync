...
Because Partners own customer communication, an authorized Partner must be able to obtain the active QR representation for its own active Ticket.
...
"credential_id": "cred_...",
"status": "ACTIVE",
"qr_payload": "opaque-renderable-value"
}
```

The exact QR cryptographic representation is deferred to the Security/Auth specification.
...
The implementation MAY derive the opaque QR payload deterministically from protected server secrets and credential identity, or use another secure recoverable design.
...
The previous credential becomes invalid and the Partner can retrieve the replacement credential through the credential endpoint.
...
Partners own checkout and payment. TktSync owns the inventory right being purchased, the authoritative Sale confirmation, Ticket/credential validity, and admission truth. Every API operation preserves that boundary.
