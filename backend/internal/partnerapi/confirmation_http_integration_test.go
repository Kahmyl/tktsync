...
type ticketingCredentialResponse struct {
...
func ticketingAssertPartnerConfirmationHTTP(
	...
	if availabilityResponse.Code !=
		http.StatusOK {
		t.Fatalf(
			"Ticketing availability status=%d body=%s",
	...
	if err := json.Unmarshal(
		...
		t.Fatalf(
			"decode Ticketing availability: %v",
			err,
		)
	}
	...
	createBody := map[string]any{
		...
		"partner_customer_ref": "ticketing-customer",
		"partner_order_ref":    "ticketing-cart",
		"buyer_session_ref":    "ticketing-browser",
	...
	if createdResponse.Code !=
		http.StatusCreated {
		t.Fatalf(
			"Ticketing create status=%d body=%s",
	...
	if checkoutResponse.Code !=
		http.StatusOK {
		t.Fatalf(
			"Ticketing checkout status=%d body=%s",
			checkoutResponse.Code,
			checkoutResponse.Body.String(),
		)
	}

	var checkout reservationCheckoutResponse
	...
	confirmBody := map[string]any{
		"checkout_attempt_id": checkout.
			CheckoutAttempt.ID,
		"partner_order_ref":   "ORD-Ticketing-HTTP",
		"partner_payment_ref": "PAY-Ticketing-HTTP",
	}
	...
	if confirmedResponse.Code !=
		http.StatusOK {
		t.Fatalf(
			"Ticketing confirm status=%d body=%s",
	...
	var confirmed ticketingConfirmationResponse
	...
	if err := json.Unmarshal(
		...
		t.Fatalf(
			"decode Ticketing confirmation: %v",
			err,
		)
	}
	...
	if confirmed.ReservationID !=
		...
		confirmed.Sale.PartnerPaymentRef !=
			"PAY-Ticketing-HTTP" {
		t.Fatalf(
			"invalid Ticketing confirmation: %s",
			confirmedResponse.Body.String(),
		)
	}
	...
	if len(confirmed.Tickets) != 3 {
		t.Fatalf(
			"Ticketing confirmation tickets=%d want=3 body=%s",
	...
	for _, ticket := range confirmed.Tickets {
		if ticket.ID == "" ||
			...
			ticket.CredentialID == "" {
			t.Fatalf(
				"invalid Ticketing Ticket result: %+v",
				ticket,
			)
		}
	}
	...
	if replay.Code != http.StatusOK {
		t.Fatalf(
			"Ticketing confirm replay status=%d body=%s",
			replay.Code,
			replay.Body.String(),
		)
	}
	...
	if !reflect.DeepEqual(
		...
		t.Fatalf(
			"Ticketing confirmation replay changed logical result: first=%s replay=%s",
			confirmedResponse.Body.String(),
			replay.Body.String(),
		)
	}
	...
	conflictBody := map[string]any{
		"checkout_attempt_id": checkout.
			CheckoutAttempt.ID,
		"partner_order_ref":   "ORD-Ticketing-CHANGED",
		"partner_payment_ref": "PAY-Ticketing-HTTP",
	}
	...
	if credentialResponse.Code !=
		http.StatusOK {
		t.Fatalf(
			"Ticketing credential status=%d body=%s",
	...
	if credentialResponse.Header().Get(
		...
		t.Fatalf(
			"Ticketing credential Cache-Control=%q want=no-store",
	...
	var credential ticketingCredentialResponse
	...
	if secondCredentialResponse.Code !=
		http.StatusOK {
		t.Fatalf(
			"Ticketing credential recovery status=%d body=%s",
			secondCredentialResponse.Code,
			secondCredentialResponse.Body.String(),
		)
	}

	var secondCredential ticketingCredentialResponse
	...
	if err != nil {
		t.Fatalf(
			"parse current Ticketing Reservation ID: %v",
			err,
		)
	}
	...
	if ticketCount != 3 {
		t.Fatalf(
			"Ticketing Ticket count=%d want=3",
			ticketCount,
		)
	}
	...
	if credentialCount != 3 {
		t.Fatalf(
			"Ticketing active credential count=%d want=3",
			credentialCount,
		)
	}
...
func ticketingAssertPartnerTicketLifecycle(
	...
	if reissueResponse.Code !=
		http.StatusOK {
		t.Fatalf(
			"Ticketing credential reissue status=%d body=%s",
	...
	var current ticketingCredentialResponse
