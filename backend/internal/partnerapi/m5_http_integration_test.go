...
type m5AvailabilityResponse struct {
...
type m5ReservationItemResponse struct {
	ID              string `json:"id"`
...
type m5ReservationResponse struct {
	ID       string                      `json:"id"`
	EventID  string                      `json:"event_id"`
	Status   string                      `json:"status"`
	Currency string                      `json:"currency"`
	Items    []m5ReservationItemResponse `json:"items"`
}

type m5CreateReservationResponse struct {
	Reservation      m5ReservationResponse `json:"reservation"`
	ReservationToken string                `json:"reservation_token"`
}

type m5CheckoutResponse struct {
...
func TestM5PartnerReservationHTTP(
	t *testing.T,
) {
	...
	eventID, err :=
		eventService.Create(
			...
			eventsvc.CreateInput{
				...
				SalesOpenAt: m5TimePtr(
					now.Add(
						-time.Hour,
					),
				),
				SalesCloseAt: m5TimePtr(
					now.Add(
						47 * time.Hour,
					),
				),
	...
	var modifiedReservation m5ReservationResponse
	...
	var checkoutBody m5CheckoutResponse
	...
	var retryCheckoutBody m5CheckoutResponse
	...
	var finalReservation m5ReservationResponse
	...
	t.Run(
		"M6ConfirmationHTTP",
		func(t *testing.T) {
			m6AssertPartnerConfirmationHTTP(
				t,
				handler,
				pool,
				credentialA,
