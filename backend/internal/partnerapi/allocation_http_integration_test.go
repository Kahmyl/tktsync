...
func TestAdminAndPartnerAllocationHTTP(
	t *testing.T,
) {
	...
	partnerA, err := partnerService.Create(
		ctx,
		adminUserID,
		"Allocation HTTP Partner A "+uuid.NewString(),
	)
	if err != nil {
	...
	partnerB, err := partnerService.Create(
		ctx,
		adminUserID,
		"Allocation HTTP Partner B "+uuid.NewString(),
	)
	if err != nil {
