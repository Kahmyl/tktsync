...
func TestTicketingConfirmationCreatesSaleTicketsAndQRCredentials(
	t *testing.T,
) {
	f := newFixture(t)
		...
		f.reservation.Confirm(
			...
			reservation.ConfirmInput{
				CheckoutAttemptID: checkout.
					CheckoutAttemptID,
				PartnerOrderRef:   "ORD-Ticketing-1",
				PartnerPaymentRef: "PAY-Ticketing-1",
			},
		)
	if err != nil {
		t.Fatalf(
	...
	if saleCount != 1 ||
		saleOrderRef == nil ||
		*saleOrderRef != "ORD-Ticketing-1" ||
		salePaymentRef == nil ||
		*salePaymentRef != "PAY-Ticketing-1" ||
...
func TestTicketingDelayedConfirmationDuringReconciliation(
	t *testing.T,
) {
	f := newFixture(t)
	...
	confirmed, err :=
		f.reservation.Confirm(
			...
			reservation.ConfirmInput{
				CheckoutAttemptID: checkout.
					CheckoutAttemptID,
				PartnerOrderRef:   "ORD-RECON-Ticketing",
				PartnerPaymentRef: "PAY-RECON-Ticketing",
			},
		)
	if err != nil {
		t.Fatalf(
...
func TestTicketingConcurrentConfirmationHasOneSale(
	t *testing.T,
) {
	f := newFixture(t)
	...
	input := reservation.ConfirmInput{
		CheckoutAttemptID: checkout.
			CheckoutAttemptID,
		PartnerOrderRef:   "ORD-CONCURRENT-Ticketing",
		PartnerPaymentRef: "PAY-CONCURRENT-Ticketing",
	}
