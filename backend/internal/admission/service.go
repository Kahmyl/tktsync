...
func (s *Service) ManualOverride(ctx context.Context, actorID uuid.UUID, input ManualOverrideInput) (ScanResult, error) {
	...
	if actorID == uuid.Nil || input.EventID == uuid.Nil || input.TicketID == uuid.Nil || input.Reason == "" || !ok {
		return ScanResult{}, apierror.New(apierror.CodeValidation, "Actor, Event, Ticket, reason, and idempotency operation are required")
	}
	var result ScanResult
	err := s.transactions.Run(ctx, func(tx pgx.Tx) error {
		var eventState string
		if err := tx.QueryRow(ctx, `SELECT state FROM events WHERE id=$1 FOR KEY SHARE`, input.EventID).Scan(&eventState); err != nil {
			return err
		}
		if err := requireAdmissionSupervisor(ctx, tx, actorID, input.EventID); err != nil {
			return err
		}
		var ticketState string
		if err := tx.QueryRow(ctx, `SELECT status FROM ticket_entitlements WHERE id=$1 AND event_id=$2 FOR UPDATE`, input.TicketID, input.EventID).Scan(&ticketState); err != nil {
			if errors.Is(err, pgx.ErrNoRows) {
		...
		if err := tx.QueryRow(ctx, `SELECT id FROM admissions WHERE ticket_entitlement_id=$1 AND status='ACTIVE' FOR UPDATE`, input.TicketID).Scan(&existing); err == nil {
			return apierror.New(apierror.CodeTicketAlreadyAdmitted, "Ticket is already admitted")
		...
		scanID, admissionID := uuid.New(), uuid.New()
		if _, err := tx.Exec(ctx, `INSERT INTO scan_attempts(id,event_id,scanner_user_id,ticket_entitlement_id,idempotency_operation_id,result,gate_reference,metadata,occurred_at) VALUES($1,$2,$3,$4,$5,'MANUAL_OVERRIDE_ADMITTED',NULLIF($6,''),jsonb_build_object('reason',$7::text),$8)`, scanID, input.EventID, actorID, input.TicketID, operationID, input.GateReference, input.Reason, now); err != nil {
			return err
		}
		if _, err := tx.Exec(ctx, `INSERT INTO admissions(id,event_id,ticket_entitlement_id,scan_attempt_id,status,admitted_at) VALUES($1,$2,$3,$4,'ACTIVE',$5)`, admissionID, input.EventID, input.TicketID, scanID, now); err != nil {
			return err
		}
		if _, err := s.audit.Append(ctx, tx, audit.Event{EventID: &input.EventID, ActorKind: audit.ActorUser, ActorUserID: &actorID, Operation: "ADMISSION_MANUAL_OVERRIDE", EntityType: "ADMISSION", EntityID: &admissionID, TicketEntitlementID: &input.TicketID, Reason: input.Reason, NewState: map[string]any{"status": "ACTIVE"}}); err != nil {
			return err
		}
		if _, err := s.outbox.Append(ctx, tx, outbox.Fact{EventID: &input.EventID, FactType: "admission.manual_override", AggregateType: "ADMISSION", AggregateID: &admissionID, Payload: map[string]any{"ticket_id": input.TicketID}}); err != nil {
			return err
		}
		display, err := ticketDisplay(ctx, tx, input.TicketID)
		if err != nil {
			return err
		}
		result = ScanResult{Result: "MANUAL_OVERRIDE_ADMITTED", ScanAttemptID: scanID, AdmissionID: &admissionID, TicketID: &input.TicketID, TicketDisplay: display, AdmittedAt: &now}
		return nil
	})
	if err != nil {
		return ScanResult{}, err
