...
func TestEventConfigurationFlow(t *testing.T) {
	...
	venueID, err := venueService.CreateVenue(
		...
		venuesvc.CreateVenueInput{
			Name:        "Configuration Integration Venue " + uuid.NewString(),
	...
	eventID, err := eventService.Create(
		...
		eventsvc.CreateInput{
			VenueID:          venueID,
			Name:             "Configuration Integration Event " + uuid.NewString(),
	...
	partnerID, err := partnerService.Create(
		ctx,
		actorID,
		"Configuration Partner "+uuid.NewString(),
	)
	if err != nil {
	...
	if err := eventService.PauseSales(ctx, actorID, eventID); err != nil {
		t.Fatalf("pause sales: %v", err)
	}
	if err := eventService.PauseSales(ctx, actorID, eventID); err == nil {
		t.Fatal("accepted an illegal duplicate pause")
	}
	if err := eventService.ResumeSales(ctx, actorID, eventID); err != nil {
		t.Fatalf("resume sales: %v", err)
	}
	if err := eventService.CloseSales(ctx, actorID, eventID); err != nil {
		t.Fatalf("close sales: %v", err)
	}
	if err := eventService.CompleteEvent(ctx, actorID, eventID); err != nil {
		t.Fatalf("complete Event: %v", err)
	}
	if err := eventService.CancelEvent(ctx, actorID, eventID, "too late"); err == nil {
		t.Fatal("accepted cancellation after completion")
	}

	cancelEventID, err := eventService.Create(ctx, actorID, eventsvc.CreateInput{
		VenueID: venueID,
		Name:    "Configuration Cancellation " + uuid.NewString(),
	})
	if err != nil {
		t.Fatalf("create cancellation Event: %v", err)
	}
	if err := eventService.CancelEvent(ctx, actorID, cancelEventID, "   "); err == nil {
		t.Fatal("accepted cancellation without an explicit reason")
	}
	if err := eventService.CancelEvent(ctx, actorID, cancelEventID, "Promoter cancellation"); err != nil {
		t.Fatalf("cancel draft Event: %v", err)
	}
	var cancelledAt *time.Time
	if err := pool.QueryRow(ctx, `SELECT cancelled_at FROM events WHERE id = $1 AND state = 'CANCELLED'`, cancelEventID).Scan(&cancelledAt); err != nil {
		t.Fatalf("read cancelled Event: %v", err)
	}
	if cancelledAt == nil {
		t.Fatal("cancelled Event did not record cancelled_at")
	}
