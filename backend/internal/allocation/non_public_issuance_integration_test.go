...
func TestTicketingNonPublicIssuance(
	t *testing.T,
) {
	...
	eventID, err :=
		eventService.Create(
			...
			eventsvc.CreateInput{
				VenueID: venueID,
				Name: "Ticketing Issuance Event " +
					uuid.NewString(),
				StartsAt: ticketingTimePtr(
					now.Add(
						48 * time.Hour,
					),
				),
				SalesOpenAt: ticketingTimePtr(
					now.Add(
						-time.Hour,
					),
				),
				SalesCloseAt: ticketingTimePtr(
					now.Add(
						47 * time.Hour,
					),
				),
	...
	allocationID, err :=
		allocationService.CreateAllocation(
			...
			allocsvc.AllocationInput{
				Mode:                   "NON_PUBLIC",
				Purpose:                "VIP",
				Reason:                 "Ticketing issuance fixture",
	...
	httpAllocationID, err :=
		allocationService.CreateAllocation(
			...
			allocsvc.AllocationInput{
				Mode:                   "NON_PUBLIC",
				Purpose:                "PRESS",
				Reason:                 "Ticketing HTTP issuance fixture",
				ReleaseDestinationKind: "SHARED",
				GATargets: []allocsvc.GATarget{
					{
						PoolID:   gaID,
	...
	adminHandler, err := adminapi.New(
		adminapi.Dependencies{
			...
			HumanAuth: func(
				...
				if token != "ticketing-admin-token" {
					return auth.HumanPrincipal{},
						io.EOF
				}

				return auth.HumanPrincipal{
					Provider: "ticketing-issuance",
					Subject: actorID.
						String(),
				}, nil
			},
	...
	if err != nil {
		t.Fatalf(
			"create Ticketing Admin handler: %v",
			err,
		)
	}
...
func ticketingAdminIssuanceRequest(
	t *testing.T,
	...
	if err != nil {
		t.Fatalf(
			"marshal Ticketing Admin request: %v",
			err,
		)
	}
...
func ticketingRequireAdminError(
	t *testing.T,
